...
public:
	...
	void reverse() {
		ForwardList reverse;	// Этот список будет задом наперед
		...
		/*Head = reverse.Head;
		*/size = reverse.size;
		*this = std::move(reverse); // Это встроенная функция, которая явным образом вызывает
		// MoveAssignment, если он есть, в противном случае behavior is undefined.
		reverse.Head = nullptr;	// Поскольку реверсный список является локальной переменнойб
		//	для него будет вызван деструктор, который полностью его очистит,
